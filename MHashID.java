//MHashID.java
//Hash table used to hold all of the movies in the database catalogged by ID number
//implemeted using double hashing

//Creates Hash class
public class MHashID implements java.io.Serializable{

	//instance variables
	private int n;
	private Mnode[] hashTable;

	//Constructor
	public MHashID() {
		hashTable = new Mnode[257];
		n = 0;
	}

	//Method that looks up and returns the index position of a node (by its key, k)
	public Mnode lookUp(int k) {
		int c = 0;
		while (c != 257) {
			if (hashTable[c] == null) {
				c++;
	...
	public void printHash() {
		...
		while (c != 257) {
			...
			if (hashTable[c] == null) {
				...
				System.out.println(hashTable[c].getID());
			}
			c++;
		}
	}
